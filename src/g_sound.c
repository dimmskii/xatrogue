	...
	if(!init)
	{
		...
		if(strlen(gamedir->string))
		{
			if (strstr(in, "mud")) //Knightmare- mud sounds are in different subdir
				sprintf(prefix,"%s/%s/sound/mud/",basedir->string,gamedir->string);
			else
		...
		{
			if (strstr(in, "mud")) //Knightmare- mud sounds are in different subdir
				sprintf(prefix,"%s/sound/mud/",basedir->string);
			else
...
qboolean FMOD_Init()
{
	char	dllname[MAX_QPATH]; // DIMCOMMENT Dima Fix Replaced _MAX_PATH. Why.
...
{
	char	filename[MAX_QPATH]; // DIMCOMMENT Dima Fix Replaced _MAX_PATH. Why.
