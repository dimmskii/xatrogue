...
void parasite_sight (edict_t *self, edict_t *other) //Knightmare- changed channel
{
...
void parasite_search (edict_t *self) //Knightmare- changed channel
{
...
void parasite_drain_attack (edict_t *self)
{
	...
	trace_t	tr;
	int damage;
	vec3_t	test_offset, test_start; //Knightmare

	AngleVectors (self->s.angles, f, r, NULL);
	VectorSet (offset, 24, 0, 6);
	G_ProjectSource (self->s.origin, offset, f, r, start);

	//Knightmare- do test trace from point above origin to enemy
	VectorSet (test_offset, 0, 0, 6);
	G_ProjectSource (self->s.origin, test_offset, f, r, test_start);
	VectorCopy (self->enemy->s.origin, end);
	tr = gi.trace (test_start, NULL, NULL, end, self, MASK_SHOT);
	tr = gi.trace (test_start, NULL, NULL, end, self, MASK_SHOT);
	if (tr.ent != self->enemy)
		return;

	//test start point- if in solid, return
	...
	VectorCopy (self->enemy->s.origin, end);
	//Knightmare- double trace here, needed to make sure clipping works
...
qboolean parasite_checkattack (edict_t *self)
{
	...
	return true; //Knightmare- default return to prevent warnings
}
...
void SP_monster_parasite (edict_t *self)
{
	...
	self->monsterinfo.search = parasite_search; //Knightmare- this was missing
